...
export type DepartmentContactFormValues = {
  department: string;
  contact: string;
  email: string;
  phone: string;
...
export function DepartmentContactForm({
  ...
  values: DepartmentContactFormValues;
}) {
  ...
  function computeChanges(fd: FormData): Change[] {
    ...
    text("contact", "Contact", values.contact);
    text("email", "Email", values.email);
    text("phone", "Phone", values.phone);
      ...
      <section id="section-main" className="flex scroll-mt-6 flex-col gap-4 rounded-xl border border-hairline bg-surface-card p-5">
        ...
        <div>
          <label className="type-caption-uppercase mb-1.5 block text-[var(--color-muted)]">
            Contact <span className="normal-case text-[11px]">(the assigned Project Manager, e.g. &quot;PM I&quot;)</span>
          </label>
          <input
            name="contact"
