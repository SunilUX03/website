...
export function NodeGraphCanvas({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const seedNodes = () => {
      ...
      const count = Math.max(20, Math.min(64, Math.round(area / 9000)));
    ...
    const drawStatic = () => {
      ...
      ctx.strokeStyle = "rgba(41, 37, 36, 0.14)";
      ctx.lineWidth = 1;
    ...
    const step = (time: number) => {
      ...
      // cursor repel/attract, contained to this canvas only
      if (pointer.active) {
        for (const n of nodes) {
      ...
      for (const n of nodes) {
        ...
        const alpha = 0.28 + brighten * 0.6;
        ctx.fillStyle = brighten > 0.05
          ? `rgba(29, 63, 143, ${alpha})`
          : `rgba(41, 37, 36, ${alpha})`;
        ctx.beginPath();
        ctx.arc(n.x, n.y, n.r + brighten * 1.5, 0, Math.PI * 2);
        ctx.fill();
      }
